...
from keras.layers import Dense, Input, BatchNormalization, Activation, Conv2D, SeparableConv2D
...
def yolo_body(inputs, num_anchors, num_classes):
    xception = Xception(input_tensor=inputs, weights='imagenet')
    xception.summary()
    ...
    # f2: 52 x 52 x 1024
    new_f2 = Concatenate()([x, f2])

    x, y2 = make_last_layers(new_f2, 256, num_anchors * (num_classes + 5))
    ...
    # f3 : 104 x 104 x 728
    new_f3 = Concatenate()([x, f3])
    x, y3 = make_last_layers(new_f3, 128, num_anchors * (num_classes + 5))
...
#     inputs = Input(shape=(832, 832, 3))
#     model = Xception(inputs)
#     model.summary()
